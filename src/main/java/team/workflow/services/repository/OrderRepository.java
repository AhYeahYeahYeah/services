...
public interface OrderRepository extends ReactiveCrudRepository<Orders, String> {
    ...
    @Query("SELECT orders.oid,account,product_num,product_name,payment,order_date,expire_date FROM orders,customer,product WHERE orders.pid=product.pid AND orders.cid=customer.cid AND orders.oid=:oid")
